...
public class TileEntityWaterWheelRender
{
    ...
    public TileEntityWaterWheelRender(Minecraft p_i1247_1_)
    {
        this.mc = p_i1247_1_;
    }
     ...
     */
    public static void renderItemIn2D(Tessellator tesselator, float maxU, float minV, float minU, float maxV, int textureWidth, int iconHeight, float modelWidth)
    {
        tesselator.startDrawingQuads(); //set to rectangle mode
        tesselator.setNormal(0.0F, 0.0F, 1.0F); //sets normal on +z axis
        //Draws one side of the item
        ...
        tesselator.draw(); //draw cached points
        
        tesselator.startDrawingQuads(); //set to rectangle mode
        tesselator.setNormal(0.0F, 0.0F, -1.0F); //sets the normal on the -z axis
        //Draws the second side of the item
        tesselator.addVertexWithUV(0.0D, 1.0D, (double)(0.0F - modelWidth), (double)maxU, (double)minV);
        tesselator.addVertexWithUV(1.0D, 1.0D, (double)(0.0F - modelWidth), (double)minU, (double)minV);
        tesselator.addVertexWithUV(1.0D, 0.0D, (double)(0.0F - modelWidth), (double)minU, (double)maxV);
        tesselator.addVertexWithUV(0.0D, 0.0D, (double)(0.0F - modelWidth), (double)maxU, (double)maxV);
        tesselator.draw(); //draw cached points
        
        
        float uOffset = 0.5F * (maxU - minU) / (float)textureWidth;
        float vOffset = 0.5F * (maxV - minV) / (float)iconHeight;
        tesselator.startDrawingQuads(); //set to rectangle mode
        tesselator.setNormal(-1.0F, 0.0F, 0.0F); //sets normal to -x axis
        int currentPixel;
        float positionalPercentage;
        float texturePosition;

        //Draws from right to left, faces pointing to the right
        for (currentPixel = 0; currentPixel < textureWidth; ++currentPixel)
        {
            positionalPercentage = (float)currentPixel / (float)textureWidth;
            texturePosition = maxU + (minU - maxU) * positionalPercentage - uOffset;
            tesselator.addVertexWithUV((double)positionalPercentage, 0.0D, (double)(0.0F - modelWidth), (double)texturePosition, (double)maxV);
            tesselator.addVertexWithUV((double)positionalPercentage, 0.0D, 0.0D, (double)texturePosition, (double)maxV);
            tesselator.addVertexWithUV((double)positionalPercentage, 1.0D, 0.0D, (double)texturePosition, (double)minV);
            tesselator.addVertexWithUV((double)positionalPercentage, 1.0D, (double)(0.0F - modelWidth), (double)texturePosition, (double)minV);
        }
        tesselator.draw(); //draw cached points
        ...
        //Draws from right to left, faces pointing to the left
        for (currentPixel = 0; currentPixel < textureWidth; ++currentPixel)
        {
            positionalPercentage = (float)currentPixel / (float)textureWidth;
            texturePosition = maxU + (minU - maxU) * positionalPercentage - uOffset;
            posWithOffset = positionalPercentage + 1.0F / (float)textureWidth;
            tesselator.addVertexWithUV((double)posWithOffset, 1.0D, (double)(0.0F - modelWidth), (double)texturePosition, (double)minV);
            tesselator.addVertexWithUV((double)posWithOffset, 1.0D, 0.0D, (double)texturePosition, (double)minV);
            tesselator.addVertexWithUV((double)posWithOffset, 0.0D, 0.0D, (double)texturePosition, (double)maxV);
            tesselator.addVertexWithUV((double)posWithOffset, 0.0D, (double)(0.0F - modelWidth), (double)texturePosition, (double)maxV);
        }
        ...
        for (currentPixel = 0; currentPixel < iconHeight; ++currentPixel)
        {
            positionalPercentage = (float)currentPixel / (float)iconHeight;
            texturePosition = maxV + (minV - maxV) * positionalPercentage - vOffset;
            posWithOffset = positionalPercentage + 1.0F / (float)iconHeight;
            tesselator.addVertexWithUV(0.0D, (double)posWithOffset, 0.0D, (double)maxU, (double)texturePosition);
            tesselator.addVertexWithUV(1.0D, (double)posWithOffset, 0.0D, (double)minU, (double)texturePosition);
            tesselator.addVertexWithUV(1.0D, (double)posWithOffset, (double)(0.0F - modelWidth), (double)minU, (double)texturePosition);
            tesselator.addVertexWithUV(0.0D, (double)posWithOffset, (double)(0.0F - modelWidth), (double)maxU, (double)texturePosition);
        }
        ...
        for (currentPixel = 0; currentPixel < iconHeight; ++currentPixel)
        {
            positionalPercentage = (float)currentPixel / (float)iconHeight;
            texturePosition = maxV + (minV - maxV) * positionalPercentage - vOffset;
            tesselator.addVertexWithUV(1.0D, (double)positionalPercentage, 0.0D, (double)minU, (double)texturePosition);
            tesselator.addVertexWithUV(0.0D, (double)positionalPercentage, 0.0D, (double)maxU, (double)texturePosition);
            tesselator.addVertexWithUV(0.0D, (double)positionalPercentage, (double)(0.0F - modelWidth), (double)maxU, (double)texturePosition);
            tesselator.addVertexWithUV(1.0D, (double)positionalPercentage, (double)(0.0F - modelWidth), (double)minU, (double)texturePosition);
        }
